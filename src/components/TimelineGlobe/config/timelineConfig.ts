export type TimelineMode = 'segment' | 'framer';

// Change this to 'segment' or 'framer' to switch between the two overall styles
const mode: TimelineMode = 'framer';

const segmentPreset = {
  globe: {
    radius: 2, // Size of the globe
    color: '#00081a', // Base color of the solid sphere
    gridColor: '#0055ff', // Color of the wireframe grid
    gridOpacity: 0.15, // How visible the grid is
    gridLines: { lat: 12, long: 24 }, // Number of latitude and longitude lines
    tilt: { x: 0.0, y: 0.0, z: 0.0 }, // Rotation of the globe
    rotates: true, // Whether the globe constantly spins
  },
  path: {
    radiusOffset: 0.05, // How far the line floats above the globe surface
    turns: 1.8, // How many times the line spirals around the globe
    verticalSpread: 0.8, // How much of the globe's height the spiral covers (0.0 to 1.0)
    totalPoints: 400, // Resolution of the line (higher = smoother curves)
    coreLine: { color: '#ffffff', thickness: 1, opacity: 0.9 }, // The inner bright line
    glowLine: { color: '#0077ff', thickness: 3, opacity: 0.4 } // The outer blurred glow
  },
  nodes: {
    activeSize: 0.08, // Size of the crystal when reached
    inactiveSize: 0.04, // Size of the crystal when in the future
    activeColor: '#aaddff', // Color of the crystal when reached
    inactiveColor: '#112244', // Color of the crystal in the future
    emissiveColor: '#0066ff', // The glow color of the active crystal
    pulseSpeed: 4, // How fast the active crystal pulses in size
    pulseRingSize: 0.15, // Base size of the expanding pulse field
    labelFontSize: '12px', // Size of the '01', '02' numbers
    labelPadding: '2px 8px', // Padding inside the number pill
    labelBg: '#1c1c1c', // Background color of the number pill
    labelActiveColor: '#aaddff', // Text color of active number
    labelInactiveColor: '#dddddd', // Text color of inactive number
  },
  cards: {
    style: 'glass', // 'glass' adds background blur, 'solid' is opaque
    scale: 1, // Change this single variable (e.g., 0.8 or 1.2) to scale the entire card
    width: '280px', // The physical width of the HTML card
    padding: '20px', // Inner spacing of the card
    background: 'rgba(10, 15, 30, 0.6)', // Card background color
    border: 'rgba(100, 150, 255, 0.15)', // Card border color
    titleSize: '18px', // Font size for the title
    descriptionSize: '14px', // Font size for the description
    gap: '16px', // Gap between text and image
    tagBg: '#0033cc', // Background of the small category pill
    tagColor: '#aaddff', // Text color of the small category pill
  },
};

const framerPreset = {
  globe: {
    radius: 1.5,
    color: '#0a080a',
    gridColor: '#00ff04',
    gridOpacity: 0.15,
    gridLines: { lat: 10, long: 20 }, // Less lines for cleaner look
    tilt: { x: 0.4, y: 0.1, z: -0.3}, // Static tilt for Framer mode
    rotates: false,
  },
  path: {
    radiusOffset: 0.05, 
    turns: 3, // Lower turns means a flatter line
    verticalSpread: 0.8, // Reduced to 0.4 to make the line flatter/closer to equator, reducing the steep angle
    totalPoints: 400,
    coreLine: { color: '#ffb3c1', thickness: 1.5, opacity: 0.9 },
    glowLine: { color: '#e83a59', thickness: 2, opacity: 0.5 }
  },
  nodes: {
    activeSize: 0.0001,
    inactiveSize: 0.00005,
    activeColor: '#ffffff',
    inactiveColor: '#ffb3c1',
    emissiveColor: '#000000',
    pulseSpeed: 1, // Slowed down pulse significantly
    pulseRingSize: 0.0, 
    
    // ==========================================
    // ⬇️ CHANGE THE NUMBERING SIZE HERE ⬇️
    // ==========================================
    labelFontSize: '4px', // <--- Change this to make the '01', '02' numbers bigger or smaller
    labelPadding: '1.5px 3.5px', // <--- Change this to make the black pill around the number bigger or smaller
    
    labelBg: '#161616', 
    labelActiveColor: '#ff4d6d', 
    labelInactiveColor: '#ffffff', 
  },
  cards: {
    style: 'solid',
    scale: 0.375, // Single variable to easily scale the solid cards
    width: '300px',
    padding: '16px',
    background: '#131315',
    border: '#262626',
    titleSize: '16px',
    descriptionSize: '13px',
    gap: '12px',
    tagBg: '#3d1620', // Brighter red so the background is clearly visible
    tagColor: '#ff4d6d',
  },
};

export const timelineConfig = {
  mode,
  ...(mode === 'segment' ? segmentPreset : framerPreset),
  milestones: [
    { 
      title: "The Static Era", 
      date: "The Past", 
      description: "Documents as static paper or PDFs. Businesses were drowning in physical files and digital equivalents that were essentially just dead storage. This was the era of manual data entry, prone to human error and massive delays. We recognized that true digital transformation couldn't happen if foundational data was trapped in static pixels.", 
      category: "Origin",
      image: "https://images.unsplash.com/photo-1568668392383-58c369615742?auto=format&fit=crop&q=80&w=600&h=400",
      progressOffset: 0.157,
    },
    { 
      title: "The Awakening", 
      date: "First 50 Clients", 
      description: "The shift to digitized, cloud-based files. We began by automating the most stubborn manual workflows for our first 50 clients, proving that enterprise documents could be managed efficiently at scale. Moving from local filing cabinets to secure, distributed cloud environments was the first step in awakening the data trapped inside.", 
      category: "Digitization",
      image: "https://images.unsplash.com/photo-1451187580459-43490279c0fa?auto=format&fit=crop&q=80&w=600&h=400",
      progressOffset: 0.300, 
    },
    { 
      title: "The Automation Engine", 
      date: "Scale Phase", 
      description: "Moving from simple storage to active parsing. By integrating early Robotic Process Automation (RPA), we didn't just store documents; we began extracting and structuring the critical data within them. Suddenly, invoices and contracts weren't just images—they were structured tables of names, dates, and amounts ready to be utilized.", 
      category: "Automation",
      image: "https://images.unsplash.com/photo-1518770660439-4636190af475?auto=format&fit=crop&q=80&w=600&h=400",
      progressOffset: 0.443, 
    },
    { 
      title: "The Turning Point", 
      date: "Launched One Ticket", 
      description: "Documents became active data streams. Launching large-scale platforms like One Ticket proved that our document infrastructure could handle millions of concurrent operations while maintaining absolute data integrity. This marked the point where our systems shifted from simple extraction tools to mission-critical operational backbones.", 
      category: "Milestone",
      image: "https://images.unsplash.com/photo-1551288049-bebda4e38f71?auto=format&fit=crop&q=80&w=600&h=400",
      progressOffset: 0.586, 
    },
    { 
      title: "The Intelligence Layer", 
      date: "AI Adoption", 
      description: "We stripped away the complexity of Natural Language Processing and Machine Learning. Our zero-code AI models began reasoning through complex, unstructured data without requiring armies of developers. Clients could now process highly variable documents instantly, moving from rigid rules to adaptive intelligence.", 
      category: "Cognitive",
      image: "https://images.unsplash.com/photo-1620712943543-bcc4688e7485?auto=format&fit=crop&q=80&w=600&h=400",
      progressOffset: 0.729, 
    },
    { 
      title: "The Interconnected Network", 
      date: "ONDC & Blockchain", 
      description: "We shifted from automating isolated companies to creating an interconnected ecosystem. By integrating with ONDC and blockchain technologies, documents now flow seamlessly and securely between entirely different B2B systems. Trust is cryptographically verified, drastically reducing the need for manual cross-company audits.", 
      category: "Ecosystem",
      image: "https://images.unsplash.com/photo-1526374965328-7f61d4dc18c5?auto=format&fit=crop&q=80&w=600&h=400",
      progressOffset: 0.871, 
    },
    { 
      title: "The Future", 
      date: "Autonomous Agents", 
      description: "Document as an Infrastructure. Our ultimate paradigm shift: an ecosystem powered entirely by autonomous AI agents. These agents don't just read data—they actively verify evidence, assess risk, and trigger downstream business workflows on their own. SequelString has transformed the humble document into the intelligent infrastructure of digital commerce.", 
      category: "Infrastructure",
      image: "https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?auto=format&fit=crop&q=80&w=600&h=400",
      progressOffset: 1.0, 
    },
  ]
};
