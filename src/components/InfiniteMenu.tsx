...
void main() {
    ...
    if (uIsZoomedIn == 1 && vInstanceId != uActiveInstanceId) {
        float b = 0.004;
        color += texture(uTex, st + vec2(b, b));
        color += texture(uTex, st + vec2(-b, -b));
        color += texture(uTex, st + vec2(-b, b));
        color += texture(uTex, st + vec2(b, -b));
        color += texture(uTex, st + vec2(b, 0.0));
        color += texture(uTex, st + vec2(-b, 0.0));
        color += texture(uTex, st + vec2(0.0, b));
        color += texture(uTex, st + vec2(0.0, -b));
        color /= 9.0;
        color.rgb *= 0.4;
    }
...
class InfiniteGridMenu {
  ...
  private discLocations!: {
    ...
    uActiveInstanceId: WebGLUniformLocation | null;
    uIsZoomedIn: WebGLUniformLocation | null;
  };
  ...
  constructor(
    ...
    let downTime = 0;
    let downPos = vec2.create();

    this.canvas.addEventListener('pointerdown', (e) => {
      downTime = performance.now();
      vec2.set(downPos, e.clientX, e.clientY);
    });

    this.canvas.addEventListener('pointerup', (e) => {
      const upTime = performance.now();
      const upPos = vec2.fromValues(e.clientX, e.clientY);
      const dist = vec2.distance(downPos, upPos);
      if (upTime - downTime < 300 && dist < 10) { 
        if (!this.isZoomedIn) {
          this.isZoomedIn = true;
          this.targetVertexIndex = this.getNearestVertexToScreenXY(upPos[0], upPos[1]);
        } else {
          // If we are already zoomed in, we can toggle out
  ...
  private init(onInit?: InitCallback): void {
    ...
    this.discLocations = {
      ...
      uIsZoomedIn: gl.getUniformLocation(this.discProgram!, 'uIsZoomedIn')
    };
  ...
  private render(): void {
    ...
    gl.uniform1i(this.discLocations.uActiveInstanceId, this.targetVertexIndex);
    gl.uniform1i(this.discLocations.uIsZoomedIn, this.isZoomedIn ? 1 : 0);
  ...
  private onControlUpdate(deltaTime: number): void {
    ...
    if (!this.isZoomedIn) {
      // Zoomed OUT state
      cameraTargetZ += Math.abs(this.control.rotationVelocity) * 80 + 2.5;
      
      this.onActiveItemChange(-1); 
      this.control.snapTargetDirection = null; 

      if (!this.control.isPointerDown) {
        // Auto-rotation when idle and zoomed out
        const autoRotateQuat = quat.setAxisAngle(quat.create(), [0, 1, 0], -0.002 * timeScale);
        quat.multiply(this.control.orientation, autoRotateQuat, this.control.orientation);
      ...
      if (!this.control.isPointerDown) {
        const targetIndex = this.targetVertexIndex !== -1 ? this.targetVertexIndex : this.findNearestVertexIndex();
        const itemIndex = targetIndex % Math.max(1, this.items.length);
        this.onActiveItemChange(itemIndex);
        const snapDirection = vec3.normalize(vec3.create(), this.getVertexWorldPosition(targetIndex));
        this.control.snapTargetDirection = snapDirection;
      } else {
        // Clear target so user can freely drag around without snapping back
        this.control.snapTargetDirection = null;
        this.targetVertexIndex = -1;
      }
    }
  ...
  private getNearestVertexToScreenXY(clientX: number, clientY: number): number {
    ...
    for (let i = 0; i < this.DISC_INSTANCE_COUNT; ++i) {
      ...
      // Only consider discs on the front half of the sphere
      if (cz < 0) continue;

      const x_prime = cx * viewProj[0] + cy * viewProj[4] + cz * viewProj[8] + viewProj[12];
      const y_prime = cx * viewProj[1] + cy * viewProj[5] + cz * viewProj[9] + viewProj[13];
      const z_prime = cx * viewProj[2] + cy * viewProj[6] + cz * viewProj[10] + viewProj[14];
      const w_prime = cx * viewProj[3] + cy * viewProj[7] + cz * viewProj[11] + viewProj[15];

      if (w_prime > 0 && z_prime < w_prime && z_prime > -w_prime) {
        const ndcX = x_prime / w_prime;
        const ndcY = y_prime / w_prime;
        
        const dist = (ndcX - x)*(ndcX - x) + (ndcY - y)*(ndcY - y);
        if (dist < minDist) {
          minDist = dist;
          nearestIndex = i;
        }
      ...
      {activeItem && (
        <>
          <div
            ...
            `}
          >
            <h2 className="text-white font-geist font-black text-4xl md:text-6xl lg:text-7xl leading-tight max-w-md drop-shadow-2xl">
              {activeItem.title}
            </h2>
          </div>
          ...
          <div
            ...
            `}
          >
            <p className="text-gray-200 font-geist text-base md:text-lg lg:text-xl max-w-[200px] md:max-w-[250px] lg:max-w-[320px] drop-shadow-2xl leading-relaxed text-right">
