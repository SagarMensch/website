...
class ArcballControl {
  ...
  private readonly AUTO_ROTATE_SPEED = 0.004;
  private _mouseIdleTimeout: ReturnType<typeof setTimeout> | null = null;
  private _mouseIsMoving = false;
  ...
  constructor(canvas: HTMLCanvasElement, updateCallback?: UpdateCallback) {
    ...
    const onDocMouseMove = () => {
      ...
      if (this._mouseIdleTimeout) clearTimeout(this._mouseIdleTimeout);
      this._mouseIdleTimeout = setTimeout(() => {
        this._mouseIsMoving = false;
        this.isHovering = false;
      }, 300);
    };
    document.addEventListener("mousemove", onDocMouseMove);
    this._docMouseMoveCleanup = () =>
      document.removeEventListener("mousemove", onDocMouseMove);

    canvas.style.touchAction = "none";
  }

  private _docMouseMoveCleanup: (() => void) | null = null;
  ...
  public update(deltaTime: number, targetFrameDuration = 16): void {
    ...
    if (this.isPointerDown) {
      ...
      if (this.isHovering && this.snapTargetDirection) {
      ...
      if (!this.isHovering) {
        ...
        quat.setAxisAngle(
          ...
          this.AUTO_ROTATE_SPEED * timeScale,
        );
...
class InfiniteGridMenu {
  ...
  constructor(
    ...
    const count = Math.max(2, this.items.length);
    this.SPHERE_RADIUS = Math.cbrt(count) * 0.55;
    this.camera.position[2] = this.SPHERE_RADIUS * 3.5 * scale;
    this.init(onInit);
  }
  ...
  public setHovering(hovering: boolean): void {
    this.control.isHovering = hovering;
  }
  ...
  private init(onInit?: InitCallback): void {
    ...
    this.discGeo = new DiscGeometry(42, 1);
    ...
    this.instancePositions = this.fibonacciSphere(this.items.length, this.SPHERE_RADIUS);
  ...
  private animate(deltaTime: number): void {
    ...
    const scale = Math.max(0.08, this.SPHERE_RADIUS * 0.22);
    const SCALE_INTENSITY = 0.2;
  ...
  private onControlUpdate(deltaTime: number): void {
    ...
    let cameraTargetZ = this.SPHERE_RADIUS * 3.5 * this.scaleFactor;
  ...
  private fibonacciSphere(count: number, radius: number): vec3[] {
    const points: vec3[] = [];
...
const InfiniteMenu: FC<InfiniteMenuProps> = ({ items = [], scale = 1.0 }) => {
  ...
  const handleVisitSite = () => {
    ...
    if (selectedItem.link.startsWith("http")) {
      ...
      console.log("Internal route:", selectedItem.link);
    }
  };
  ...
  return (
    <div className="relative w-full h-full overflow-hidden">
      <canvas
        ...
        ref={canvasRef}
        className="cursor-grab w-full h-full overflow-hidden relative outline-none active:cursor-grabbing top-1"
        style={{
          transition:
            "transform 0.6s cubic-bezier(0.25,0.1,0.25,1), opacity 0.6s cubic-bezier(0.25,0.1,0.25,1)",
          transform: isDetailOpen ? "scale(0.8)" : "scale(1)",
      ...
      <div
        ...
        style={{
          ...
          transform: isDetailOpen ? "scale(1)" : "scale(1.1)",
          pointerEvents: isDetailOpen ? "auto" : "none",
        }}
      >
        <button
          onClick={handleClose}
          className="absolute top-20 right-60 z-[60] w-10 h-10 flex items-center justify-center rounded-full bg-white/10 hover:bg-white/20 transition-colors duration-200 text-white text-xl cursor-pointer"
        >
          &#x2715;
        </button>
          ...
          {selectedItem && (
            <img
              ...
              style={{
                ...
                transform: isDetailOpen ? "scale(1)" : "scale(0.5)",
              }}
            />
          )}
        </div>
        ...
        <div className="w-1/2 h-full flex flex-col justify-center pr-20">
          {selectedItem && (
            <>
              <h2
                className="text-white font-black text-5xl mb-6"
                style={{
                  transition:
              ...
              <button
                ...
                className="self-start px-8 py-3 bg-[#00ffff] text-black font-semibold rounded-full hover:bg-white transition-colors duration-200 text-sm tracking-wide cursor-pointer"
                style={{
                  transition:
